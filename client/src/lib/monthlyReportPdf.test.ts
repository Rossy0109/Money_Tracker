...
describe("monthly report PDF", () => {
  ...
  it("embeds the Bengali font and downloads a month-specific BDT report", async () => {
    ...
    expect(documentApi.addFileToVFS).toHaveBeenCalledWith(
      "NotoSansBengali-Regular.ttf",
      expect.any(String)
    );
    expect(documentApi.text).toHaveBeenCalledWith(
    ...
    expect(documentApi.text).toHaveBeenCalledWith(
      "১ টি লেনদেন | উপমোট: ৳ ৩,৫০০ | অংশ: ১০০%",
      expect.any(Number),
      expect.any(Number),
      { align: "right" }
    );
    ...
    expect(documentApi.splitTextToSize).toHaveBeenCalledWith(
      "আগস্ট মাসের বেতন পরিশোধ",
      320
    );
  ...
  it("continues a long detailed transaction list onto a new PDF page", async () => {
    ...
    expect(documentApi.text).toHaveBeenCalledWith(
      "ব্যয়ের বিস্তারিত লেনদেন (চলমান)",
      52,
      expect.any(Number)
    );
    expect(documentApi.text).toHaveBeenCalledWith(
      "ক্যাটাগরি: বেতন (চলমান)",
      50,
      expect.any(Number)
    );
    expect(documentApi.text).toHaveBeenCalledWith(
      "২৮ টি লেনদেন | উপমোট: ৳ ৪২,০০০ | অংশ: ১০০%",
      expect.any(Number),
      expect.any(Number),
      { align: "right" }
    );
  ...
  it("highlights the three highest expense categories in ranked order, including a tie", async () => {
    await downloadMonthlyReportPdf({
      ...
      transactionDetails: [
        ...
        },
        {
          ...
          type: "expense",
          categoryName: "C",
          description: "দ্বিতীয় শীর্ষ ব্যয়",
          amount: 5000,
        },
        {
          occurredAt: new Date("2026-08-19T12:00:00.000Z"),
          ...
          type: "expense",
          categoryName: "D",
          description: "তৃতীয় শীর্ষ ব্যয়",
          amount: 3000,
        },
      ],
    });
    ...
    expect(documentApi.text).toHaveBeenCalledWith(
      "গত মাস (জুলাই ২০২৬): ৳ ৩,০০০ | বেড়েছে ৳ ২,০০০",
      58,
      expect.any(Number)
    );
    expect(documentApi.text).toHaveBeenCalledWith(
      "গত মাস (জুলাই ২০২৬): ৳ ৭,০০০ | কমেছে ৳ ২,০০০",
      58,
      expect.any(Number)
    );
    expect(documentApi.text).not.toHaveBeenCalledWith(
      "4. A",
      54,
      expect.any(Number)
    );
  ...
  it("exports dedicated profit-and-loss and financial-position reports", async () => {
    const report = {
      ...
      netAmount: 8500,
      categoryTotals: [
        { name: "Business", type: "income" as const, total: 12000 },
      ],
      totalDebt: 900,
      totalReceivable: 1400,
      transactionCount: 1,
    ...
    expect(documentApi.text).toHaveBeenCalledWith(
      "অ্যাকাউন্টভিত্তিক ব্যালেন্স",
      52,
      expect.any(Number)
    );
    expect(documentApi.save).toHaveBeenCalledWith(
      "financial-position-report-2026-08.pdf"
    );
  });
  ...
  it("uses the device share sheet only when the browser supports PDF file sharing", async () => {
    ...
    await expect(shareMonthlyReportPdf(report, "income")).resolves.toBe(
      "shared"
    );
    expect(documentApi.output).toHaveBeenCalledWith("blob");
    expect(share).toHaveBeenCalledWith(
      expect.objectContaining({ title: "আয়ের রিপোর্ট" })
    );
    expect(share.mock.calls[0][0].files[0].name).toBe(
      "income-report-2026-08.pdf"
    );
    ...
    await downloadMonthlyReportPdf({
      ...
      transactionDetails: [
        ...
        },
        {
          ...
          type: "expense",
          categoryName: "বেতন",
          description: "আগস্ট মাসের বেতন",
          amount: 3500,
        },
      ],
    });

    const incomeHeadingIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "আয়ের বিস্তারিত লেনদেন"
    );
    const expenseHeadingIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "ব্যয়ের বিস্তারিত লেনদেন"
    );
    const incomeVoucherIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "V-011"
    );
    const expenseVoucherIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "V-012"
    );
  ...
  it("groups transaction rows under one category subheading before the next category", async () => {
      ...
      transactionDetails: [
        {
          ...
          type: "income",
          categoryName: "Business",
          description: "প্রথম ব্যবসায়িক আয়",
          amount: 5000,
        },
        {
          occurredAt: new Date("2026-08-18T12:00:00.000Z"),
        ...
        },
        {
          ...
          type: "income",
          categoryName: "Business",
          description: "দ্বিতীয় ব্যবসায়িক আয়",
          amount: 5000,
        },
      ],
    });

    const businessHeadingIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "ক্যাটাগরি: Business"
    );
    const salaryHeadingIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "ক্যাটাগরি: Salary"
    );
    const firstBusinessVoucherIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "V-010"
    );
    const secondBusinessVoucherIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "V-012"
    );
    const salaryVoucherIndex = documentApi.text.mock.calls.findIndex(
      call => call[0] === "V-011"
    );
    ...
    expect(secondBusinessVoucherIndex).toBeGreaterThan(
      firstBusinessVoucherIndex
    );
    ...
    expect(documentApi.text).toHaveBeenCalledWith(
      "২ টি লেনদেন | উপমোট: ৳ ১০,০০০ | অংশ: ৭৬.৯%",
      expect.any(Number),
      expect.any(Number),
      { align: "right" }
    );
    expect(documentApi.text).toHaveBeenCalledWith(
      "১ টি লেনদেন | উপমোট: ৳ ৩,০০০ | অংশ: ২৩.১%",
      expect.any(Number),
      expect.any(Number),
      { align: "right" }
    );
      ...
      transactionDetails: [
        {
          ...
          type: "income",
          categoryName: "Business",
          description: "শূন্য মূল্যের সংশোধনী",
          amount: 0,
        },
      ],
    });
