...
describe("administrator audit-filter behavior", () => {
  ...
  it("clears all optional query filters and never includes a password fallback", () => {
    expect(buildAdminAuditFilterInput("", EMPTY_AUDIT_FILTERS)).toEqual({ from: undefined, to: undefined, actorUserId: undefined, actorRole: undefined, search: undefined });
    expect(buildAdminAuditFilterInput("pending", EMPTY_AUDIT_FILTERS)).not.toHaveProperty("password");
  });
});
