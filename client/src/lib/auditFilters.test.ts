...
describe("administrator audit-filter behavior", () => {
  it("converts selected date and user controls into a stable protected query input", () => {
    const input = buildAdminAuditFilterInput("secret", { from: new Date("2026-08-01T12:00:00.000Z"), to: new Date("2026-08-19T12:00:00.000Z"), actorUserId: "17", search: "  transaction  " });
  ...
  it("clears all optional query filters while retaining the protected fallback password", () => {
    expect(buildAdminAuditFilterInput("", EMPTY_AUDIT_FILTERS)).toEqual({ password: "pending", from: undefined, to: undefined, actorUserId: undefined, search: undefined });
  });
});
