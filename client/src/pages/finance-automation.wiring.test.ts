...
const automationSource = readFileSync(
  resolve(process.cwd(), "client/src/pages/FinanceAutomation.tsx"),
  "utf8"
);
...
describe("finance automation workspace wiring", () => {
  it("uses the selected project for recurring templates, reminders, and debt or receivable ageing", () => {
    expect(automationSource.replace(/\s+/g, "")).toContain(
      "trpc.finance.automationOverview.useQuery({projectId}"
    );
    expect(automationSource).toContain(
      "trpc.finance.addRecurringTemplate.useMutation"
    );
    expect(automationSource).toContain(
      "trpc.finance.enableBillReminder.useMutation"
    );
    expect(automationSource).toContain(
      "trpc.finance.generateRecurringNow.useMutation"
    );
