...
export default function FamilyHousehold() {
  ...
  const { data: households = [], isLoading: householdsLoading } =
    trpc.finance.households.useQuery();
  ...
  const [chartExporting, setChartExporting] = useState<"image" | "pdf" | null>(
    null
  );
  ...
  const canContribute =
    overview?.currentRole === "owner" || overview?.currentRole === "editor";
  const currentMonth = useMemo(() => new Date().toISOString().slice(0, 7), []);
  const contributorSpend = overview?.contributorSpend ?? [];
  const contributorTotal = contributorSpend.reduce(
  ...
  const monthlyChartData = useMemo(
    () =>
      monthlyContributorSpend?.months.map(month => ({
        monthKey: month.monthKey,
        ...Object.fromEntries(
          month.contributors.map(item => [
            `member-${item.contributorUserId}`,
            item.amount,
          ])
        ),
      })) ?? [],
  ...
  const monthlyComparisonTotal =
    monthlyContributorSpend?.months.reduce(
  ...
  const refresh = async () => {
    await Promise.all([
      ...
      householdId !== null
        ? utils.finance.householdOverview.invalidate({ householdId })
        : Promise.resolve(),
    ]);
  };
  const exportMonthlyComparison = async (kind: "image" | "pdf") => {
    if (!monthlyChartExportRef.current || monthlyContributors.length === 0)
      return;
    setChartExporting(kind);
    try {
      const { downloadHouseholdChartImage, downloadHouseholdChartPdf } =
        await import("@/lib/householdChartExport");
      if (kind === "image")
        await downloadHouseholdChartImage(monthlyChartExportRef.current);
      else
        await downloadHouseholdChartPdf(monthlyChartExportRef.current, {
          familyName: overview?.household.name ?? "",
          title: pdfTitle.trim() || defaultPdfTitle,
        });
      toast.success(
        kind === "image"
          ? "চার্টের ছবি ডাউনলোড প্রস্তুত"
          : "পরিবারের নামসহ PDF ডাউনলোড প্রস্তুত"
      );
    } catch (error) {
      ...
      <main className="space-y-5 sm:space-y-6">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">
                একসঙ্গে বাজেট, আলাদা দায়িত্ব
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#d4e9da]">
                ব্যক্তিগত প্রজেক্ট অপরিবর্তিত রেখে পরিবারের সদস্য, ভূমিকা এবং
                মাসিক শেয়ার করা বাজেট পরিচালনা করুন।
              </p>
            </div>
            {households.length > 0 && (
              <Select
                value={String(effectiveHouseholdId ?? households[0].id)}
                onValueChange={value => setHouseholdId(Number(value))}
              >
                <SelectTrigger className="h-11 w-full border-white/25 bg-white/10 text-white sm:w-64">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {households.map(item => (
              ...
              {invitations.map(item => (
                ...
                >
                  <span>
                    ...
                    {roleLabel(item.role)} হিসেবে আমন্ত্রণ
                  </span>
                  <Button
                    size="sm"
        ...
        <Card className="border-[#dce9df] shadow-sm">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              আপনি পরিচালক হিসেবে যুক্ত হবেন; ব্যক্তিগত প্রজেক্টের তথ্য এতে
              যুক্ত হবে না।
            </CardDescription>
          </CardHeader>
            ...
            <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
              <Card className="border-[#dce9df]">
                <CardHeader>
                  ...
                  <CardDescription>
                    আপনার ভূমিকা:{" "}
                    <strong>{roleLabel(overview.currentRole)}</strong>
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {overview.owner && (
                    <MemberRow
                      name={
                        overview.owner.name || overview.owner.email || "পরিচালক"
                      }
                  ...
                  {overview.members.map(member => (
                    ...
                    >
                      <MemberRow
                        ...
                        email={member.userEmail || member.inviteeEmail}
                        role={member.role}
              ...
              <Card className="border-[#dce9df]">
                <CardHeader>
                  ...
                  <CardDescription>
                    শুধু পরিচালক সদস্যের ইমেইলে আমন্ত্রণ পাঠাতে পারেন।
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {canManage ? (
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="editor">
                            সম্পাদক — খরচ যোগ করতে পারবেন
                          </SelectItem>
                          <SelectItem value="viewer">
                            দর্শক — শুধু দেখতে পারবেন
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      <Button
            ...
            <section className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
              <Card className="border-[#dce9df]">
                <CardHeader>
                  ...
                  <CardDescription>
                    {currentMonth} মাসের পরিবারভিত্তিক বরাদ্দ ও খরচ।
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {overview.sharedBudgets.length ? (
              ...
              <Card className="border-[#dce9df]">
                <CardHeader>
                  ...
                  <CardDescription>
                    এই খরচ শুধু নির্বাচিত পারিবারিক প্রোফাইলের শেয়ার করা বাজেটে
                    যুক্ত হবে।
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-[#dce9df] shadow-sm">
                <CardHeader className="flex flex-col gap-3 pb-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {currentMonth} মাসে শেয়ার করা বাজেট থেকে কে কত খরচ যোগ
                      করেছেন তার সারাংশ।
                    </CardDescription>
                  </div>
                ...
                <CardContent>
                  {contributorSpend.length > 0 ? (
                    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(280px,.65fr)] xl:items-start">
                      <div
                        className="min-h-60"
                        style={{
                          height: `${Math.max(240, contributorSpend.length * 52)}px`,
                        }}
                      >
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart
                            data={contributorSpend}
                            layout="vertical"
                          ...
                          >
                            ...
                            <XAxis
                              type="number"
                              tick={{ fill: "#668075", fontSize: 12 }}
                              axisLine={false}
                              tickLine={false}
                              tickFormatter={value =>
                                `৳${new Intl.NumberFormat("bn-BD", { notation: "compact", maximumFractionDigits: 1 }).format(Number(value))}`
                              }
                            />
                            <YAxis
                              type="category"
                              dataKey="contributorName"
                              width={92}
                              tick={{ fill: "#315a49", fontSize: 12 }}
                              axisLine={false}
                              tickLine={false}
                            />
                            <Tooltip
                              cursor={{ fill: "#f2f8f3" }}
                              formatter={value => [taka(Number(value)), "খরচ"]}
                              labelFormatter={label => `${label} এর খরচ`}
                              contentStyle={{
                                borderRadius: 12,
                                borderColor: "#cfe2d3",
                                color: "#173f36",
              ...
              <Card className="border-[#dce9df] bg-white shadow-sm">
                <CardHeader className="flex flex-col gap-3 pb-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <CardDescription>
                      গত ৬ মাসে শেয়ার করা বাজেটে প্রত্যেক সদস্য কত খরচ যোগ
                      করেছেন তার মাসভিত্তিক তুলনা।
                    </CardDescription>
                  </div>
                  {monthlyContributors.length > 0 && (
                    <div
                      className="w-full space-y-2 sm:w-auto"
                      data-chart-export-hide
                    >
                          ...
                          >
                            ...
                            {monthlyContributors.map((member, index) => (
                              <Bar
                                ...
                                dataKey={`member-${member.contributorUserId}`}
                                name={member.contributorName}
                                fill={
                                  comparisonColors[
                                    index % comparisonColors.length
                                  ]
                                }
                                radius={[6, 6, 0, 0]}
                                maxBarSize={34}
