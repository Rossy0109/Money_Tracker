...
const initialRecurring = (): RecurringForm => ({
  ...
  note: "",
  frequency: "monthly",
  scheduleDay: String(new Date().getDate()),
  nextRunAt: new Date().toISOString().slice(0, 10),
});

const SUBSCRIPTION_PRESETS = [
  {
    name: "ইন্টারনেট / WiFi বিল",
    type: "expense" as const,
    amount: "1000",
    paymentMethod: "bKash",
    frequency: "monthly" as const,
    note: "মাসিক ইন্টারনেট বিল",
  },
  {
    name: "বিদ্যুৎ বিল (DESCO/DPDC)",
    type: "expense" as const,
    amount: "1500",
    paymentMethod: "bKash",
    frequency: "monthly" as const,
  ...
  },
  {
    ...
    frequency: "monthly" as const,
    note: "মাসিক বাসা ভাড়া",
  },
  {
    name: "Netflix / OTT সাবস্ক্রিপশন",
    type: "expense" as const,
    amount: "800",
    paymentMethod: "কার্ড",
    frequency: "monthly" as const,
    note: "মাসিক স্ট্রিমিং সাবস্ক্রিপশন",
  },
  {
    name: "মোবাইল রিচার্জ / প্যাক",
    type: "expense" as const,
    amount: "500",
    paymentMethod: "bKash",
    frequency: "monthly" as const,
    note: "মাসিক মোবাইল বিল",
  },
  {
    name: "মাসিক বেতন আয়",
...
export default function FinanceAutomation() {
  ...
  const { data: overview } = trpc.finance.overview.useQuery(
    { projectId },
    { enabled: projectId > 0 }
  );
  const { data: automation, isLoading } =
    trpc.finance.automationOverview.useQuery(
      { projectId },
      { enabled: projectId > 0 }
    );
  const refresh = async () => {
    await Promise.all([
      utils.finance.automationOverview.invalidate({ projectId }),
      utils.finance.overview.invalidate({ projectId }),
    ]);
  };
  const addRecurring = trpc.finance.addRecurringTemplate.useMutation({
    onSuccess: async () => {
      toast.success("পুনরাবৃত্ত লেনদেন তৈরি হয়েছে");
      setForm(initialRecurring());
      await refresh();
    },
    onError: error =>
      toast.error(error.message || "পুনরাবৃত্ত লেনদেন তৈরি করা যায়নি"),
  });
  const runNow = trpc.finance.generateRecurringNow.useMutation({
    onSuccess: async result => {
      await refresh();
      toast.success(
        result.created
          ? `${new Intl.NumberFormat("bn-BD").format(result.created)}টি নির্ধারিত লেনদেন যোগ হয়েছে`
          : "আজ নতুন কোনো নির্ধারিত লেনদেন নেই"
      );
    },
    onError: error => toast.error(error.message),
  });
  const toggleRecurring = trpc.finance.setRecurringActive.useMutation({
    onSuccess: refresh,
    onError: error => toast.error(error.message),
  });
  const enableBill = trpc.finance.enableBillReminder.useMutation({
    onSuccess: async () => {
      await refresh();
      toast.success("বিলের দৈনিক স্মরণ পরীক্ষা চালু হয়েছে");
    },
    onError: error => toast.error(error.message || "বিল স্মরণ চালু করা যায়নি"),
  });
  ...
  const applyPreset = (preset: (typeof SUBSCRIPTION_PRESETS)[0]) => {
    const matchedCategory = overview?.categories.find(
      c =>
        c.type === preset.type &&
        (c.name.includes("বিল") ||
          c.name.includes("বেতন") ||
          c.name.includes("বাসা") ||
          c.name.includes("যাতায়াত") ||
  ...
  const submitRecurring = (event: FormEvent) => {
    ...
    if (!projectId || !form.categoryId || !form.amount || !form.nextRunAt)
      return;
    addRecurring.mutate({
      projectId,
      ...
      scheduleDay: Number(form.scheduleDay),
      nextRunAt: new Date(`${form.nextRunAt}T12:00:00`),
    });
  };
  const relevantCategories =
    overview?.categories.filter(category => category.type === form.type) ?? [];
  if (projectsLoading)
    return (
      <DashboardLayout>
        ...
        <header className="flex flex-col gap-4 rounded-[1.75rem] bg-[#173f36] p-5 text-white shadow-[0_20px_50px_rgba(18,60,50,.16)] sm:flex-row sm:items-end sm:justify-between sm:p-7">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold sm:text-3xl">
              পুনরাবৃত্ত লেনদেন ও বিল স্মরণ
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#d6e9dc]">
              সময়সূচি আপনার অনুমতিতে চলে। কোনো নির্ধারিত লেনদেন একাধিকবার যোগ
              হয় না।
            </p>
          </div>
            ...
            <Card className="border-[#dbe7dd] bg-[#f9fbf9]">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-xs">
                  বহুল ব্যবহৃত মাসিক বিল ও সাবস্ক্রিপশনের তথ্য এক ক্লিকে ফর্মে
                  বসাতে নিচের যেকোনো বাটনে ট্যাপ করুন।
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <section className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
              <Card className="border-[#dbe7dd]">
                <CardHeader>
                  ...
                  <CardDescription>
                    সাপ্তাহিক বা মাসিক আয়-ব্যয় নির্ধারণ করুন। প্রতিদিন সকালে
                    স্বয়ংক্রিয় যাচাই নির্ধারিত লেনদেন তৈরি করবে।
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <form
                    ...
                    <label className="grid gap-1 text-xs font-semibold text-[#4b6c60]">
                      ...
                      <select
                        ...
                        onChange={event =>
                          setForm(current => ({
                            ...current,
                            type: event.target.value as RecurringForm["type"],
                            categoryId: "",
                          }))
                        }
              ...
              <Card className="border-[#dbe7dd]">
                <CardHeader>
                  ...
                  <CardDescription>
                    অপরিশোধিত বিলের নির্ধারিত দিনের আগে প্রতিদিন সকালে
                    স্বয়ংক্রিয় স্মরণ পরীক্ষা করা হয়।
                  </CardDescription>
                </CardHeader>
                      ...
                      .map(bill => (
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <Button
                              size="sm"
                              className="h-10 rounded-xl bg-[#a96518] hover:bg-[#845111]"
                              disabled={enableBill.isPending}
                              onClick={() =>
                                enableBill.mutate({
                                  projectId,
                                  id: bill.id,
            ...
            <section className="grid gap-5 xl:grid-cols-[1.08fr_.92fr]">
              <Card className="border-[#dbe7dd]">
                <CardHeader>
                  ...
                  <CardDescription>
                    প্রিভিউ দেখে প্রয়োজনে একবার হাতে চালান বা সাময়িক বিরতি দিন।
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {automation?.recurring.length ? (
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge
                                variant={
                                  item.isActive ? "secondary" : "outline"
                                }
                              >
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              disabled={runNow.isPending || !item.isActive}
                              onClick={() =>
                                runNow.mutate({ projectId, id: item.id })
                              }
                            >
              ...
              <Card className="border-[#dbe7dd]">
                <CardHeader>
                  ...
                  <CardDescription>
                    বকেয়ার বয়স ও পরিশোধের তারিখ ধরে অগ্রাধিকার ঠিক করুন।
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {automation?.ageing.length ? (
                    automation.ageing.map(item => (
                      <article
                        key={item.id}
                        className="rounded-2xl border border-[#e8e3f1] p-4"
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          >
                            {item.status === "overdue_31_plus"
                              ? "৩১+ দিন বকেয়া"
                              : item.status === "overdue_1_30"
                                ? `${new Intl.NumberFormat("bn-BD").format(item.daysOverdue || 0)} দিন বকেয়া`
                                : item.status === "due_today"
                                  ? "আজ পরিশোধযোগ্য"
                                  : item.status === "undated"
                                    ? "তারিখ দিন"
