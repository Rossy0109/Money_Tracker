...
export default function FinanceInsights() {
  ...
  const { data: plan, isLoading: planLoading } =
    trpc.finance.budgetPlan.useQuery(
      { projectId, monthKey: planMonth },
      { enabled: projectId > 0 }
    );
  const { data: analytics, isLoading: analyticsLoading } =
    trpc.finance.analytics.useQuery(
      { projectId, months: 6 },
      { enabled: projectId > 0 }
    );
  const searchInput = useMemo(
    () => ({
      ...
      categoryId: appliedSearch.categoryId
        ? Number(appliedSearch.categoryId)
        : undefined,
      type: appliedSearch.type === "all" ? undefined : appliedSearch.type,
      from: appliedSearch.from
        ? new Date(`${appliedSearch.from}T00:00:00`)
        : undefined,
      to: appliedSearch.to
        ? new Date(`${appliedSearch.to}T23:59:59.999`)
        : undefined,
      minAmount: appliedSearch.minAmount
        ? Number(appliedSearch.minAmount)
        : undefined,
      maxAmount: appliedSearch.maxAmount
        ? Number(appliedSearch.maxAmount)
        : undefined,
      limit: 100,
    }),
  ...
  const { data: results = [], isFetching: searching } =
    trpc.finance.searchTransactions.useQuery(searchInput, {
      enabled: projectId > 0,
    });
  const saveBudget = trpc.finance.saveBudget.useMutation({
    onSuccess: async (_result, input) => {
      await Promise.all([
        utils.finance.budgetPlan.invalidate({
        ...
        <header className="flex flex-col gap-4 rounded-[1.75rem] bg-[#123c32] p-5 text-white shadow-[0_20px_50px_rgba(18,60,50,.16)] sm:flex-row sm:items-end sm:justify-between sm:p-7">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold sm:text-3xl">
              আগাম পরিকল্পনা, প্রবণতা ও দ্রুত খোঁজ
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#d6e9dc]">
              প্রতিটি ফলাফল কেবল নির্বাচিত হিসাবখাতার তথ্য ব্যবহার করে তৈরি হয়।
            </p>
          </div>
        ...
        {!projects.length ? (
          <Card>
            <CardContent className="p-7 text-center text-sm text-muted-foreground">
              হিসাবখাতা প্রস্তুত হচ্ছে। কিছুক্ষণ পর আবার চেষ্টা করুন।
            </CardContent>
          </Card>
        ) : (
          <>
            <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
                ...
                <CardHeader className="gap-3 border-b border-[#edf2ee] pb-5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      আগের মাসের খরচ ও বাজেট দেখে প্রস্তাব আসে; আপনার ক্লিক ছাড়া
              ...
              <Card className="border-[#dbe7dd] shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    নির্বাচিত হিসাবখাতার সাম্প্রতিক অবস্থা।
                  </CardDescription>
                </CardHeader>
            ...
            <Card className="border-[#dbe7dd] shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  আয় থেকে ব্যয় বাদ দিয়ে সঞ্চয়ের পরিমাণ হিসাব করা হয়েছে। বাজেট
                  ব্যবহারও একই মাসের বাজেটের বিপরীতে দেখানো হয়।
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5">
                <div
                  className="h-64 w-full"
                  aria-label="ছয় মাসের আর্থিক প্রবণতা চার্ট"
                >
                  {analyticsLoading ? (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="secondary">
                          {point.budgetUsagePercentage === null
                            ? "বাজেট নেই"
            ...
            <Card className="border-[#dbe7dd] shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  বিবরণ, স্বয়ংক্রিয় ভাউচার নম্বর, ক্যাটাগরি, তারিখ এবং টাকার
                  পরিমাণ দিয়ে নির্বাচিত হিসাবখাতার লেনদেন খুঁজুন।
                </CardDescription>
              </CardHeader>
