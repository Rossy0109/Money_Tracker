...
const dataLayerSource = readFileSync(
...
describe("audit logging coverage", () => {
  it("records every protected create, update, and delete data mutation through the immutable audit helper", () => {
    for (const mutation of auditedMutations) {
      const start = dataLayerSource.indexOf(
      ...
      const implementation = dataLayerSource.slice(
        start,
        nextFunction === -1 ? undefined : nextFunction
      );
      expect(
        start,
        `${mutation} should exist in the finance data layer`
      ).toBeGreaterThanOrEqual(0);
      expect(
        implementation,
        `${mutation} should write an audit entry`
      ).toContain("await logAudit(");
    }
  });
});
