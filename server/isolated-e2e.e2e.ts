...
function caller(
  user: E2eUser,
  adminElevation: AdminElevationPayload | null = null
) {
...
beforeAll(async () => {
  ...
  const rows = await db
    .select()
    .from(users)
    .where(inArray(users.openId, [...OPEN_IDS]));
  ...
  if ([owner, editor, viewer, outsider, administrator].some(user => !user))
    throw new Error("E2E পরিচয় তৈরি করা যায়নি");
...
describe("isolated role, invitation, and restoration E2E", () => {
  it("enforces platform administrator access through the real tRPC procedure and isolated database", async () => {
    ...
    if (!password)
      throw new Error(
        "ADMIN_ACCESS_PASSWORD ছাড়া administrator E2E পরীক্ষা চালানো যায় না"
      );

    await expect(caller(owner).admin.users()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    // Elevated procedures additionally require the password-verified session.
    const verified = await caller(administrator).admin.verifyAccess({
      password,
    });
    ...
    await expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({ email: "owner@e2e.test", role: "user" }),
      ])
    );
  });
  ...
  it("enforces owner, editor, viewer, and normalized-email invitation boundaries end to end", async () => {
    ...
    const created = await ownerCaller.finance.createHousehold({
      name: "E2E পারিবারিক প্রোফাইল",
    });
    const householdId = created.household.id;

    await expect(
      viewerCaller.finance.householdOverview({ householdId })
    ).rejects.toThrow("এই পারিবারিক প্রোফাইলে আপনার অনুমতি নেই");
    await expect(
      editorCaller.finance.inviteHouseholdMember({
        householdId,
        email: "viewer@e2e.test",
        role: "viewer",
      })
    ...
    const editorInvitationId = await ownerCaller.finance.inviteHouseholdMember({
      householdId,
      email: " Editor@E2E.Test ",
      displayName: "সম্পাদক",
      role: "editor",
    ...
    await expect(
      outsiderCaller.finance.acceptHouseholdInvitation({
        membershipId: editorInvitationId,
      })
    ).rejects.toThrow("এই আমন্ত্রণ গ্রহণের অনুমতি আপনার নেই");
    expect(await editorCaller.finance.householdInvitations()).toEqual(
      expect.arrayContaining([
    ...
    const editorOverview = await editorCaller.finance.acceptHouseholdInvitation(
      { membershipId: editorInvitationId }
    );
    ...
    ).rejects.toThrow("এই কাজটি করার অনুমতি আপনার নেই");
    await expect(
      ownerCaller.finance.inviteHouseholdMember({
        householdId,
        email: "editor@e2e.test",
        role: "editor",
      })
    ).rejects.toThrow("এই সদস্য ইতিমধ্যে যুক্ত আছেন");
    await expect(
      editorCaller.finance.saveSharedHouseholdBudget({
        householdId,
        label: "খাদ্য",
    ...
    await ownerCaller.finance.saveSharedHouseholdBudget({
      householdId,
      label: "খাদ্য",
      monthKey: currentMonth,
      amount: 1000,
    });
    const ownerOverview = await ownerCaller.finance.householdOverview({
      householdId,
    });
    const budgetId = ownerOverview.sharedBudgets.find(
      budget => budget.label === "খাদ্য"
    )?.id;
    expect(budgetId).toBeTypeOf("number");
    await editorCaller.finance.addSharedHouseholdExpense({
      householdId,
      budgetId: budgetId!,
      amount: 250,
      note: "E2E editor entry",
      occurredAt: new Date(),
    });

    const viewerInvitationId = await ownerCaller.finance.inviteHouseholdMember({
      householdId,
      email: "viewer@e2e.test",
      displayName: "দর্শক",
      role: "viewer",
    });
    const viewerOverview = await viewerCaller.finance.acceptHouseholdInvitation(
      { membershipId: viewerInvitationId }
    );
    expect(viewerOverview.currentRole).toBe("viewer");
    await expect(
      viewerCaller.finance.addSharedHouseholdExpense({
        householdId,
        budgetId: budgetId!,
        amount: 50,
        note: "viewer blocked",
    ...
    const refreshedOwnerOverview = await ownerCaller.finance.householdOverview({
      householdId,
    });
    expect(refreshedOwnerOverview.recentExpenses).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          contributorUserId: editor.id,
          amount: "250.00",
        }),
      ])
    );
  ...
  it("previews backups without mutation, requires confirmation, restores into a new project, and preserves the source project", async () => {
    ...
    const sourceProject = await adminCaller.projects.create({
      name: "E2E উৎস হিসাবখাতা",
    });
    const sourceProjectId = sourceProject.id;
    await adminCaller.finance.addAccount({
      projectId: sourceProjectId,
      name: "E2E নগদ",
      type: "cash",
      openingBalance: 1000,
    });
    const preparedSource = await adminCaller.finance.overview({
      projectId: sourceProjectId,
    });
    const expenseCategory = preparedSource.categories.find(
      category => category.type === "expense"
    );
    const account = preparedSource.accounts.find(
      item => item.name === "E2E নগদ"
    );
    expect(expenseCategory?.id).toBeTypeOf("number");
    expect(account?.id).toBeTypeOf("number");
    await adminCaller.finance.addTransaction({
      projectId: sourceProjectId,
      ...
      amount: 125,
      paymentMethod: "cash",
      note: "E2E পুনরুদ্ধার উৎস",
      occurredAt: new Date(),
    });

    const backup = await adminCaller.finance.exportProjectBackup({
      projectId: sourceProjectId,
    });
    const preview = await adminCaller.finance.previewProjectBackup({ backup });
    expect(preview).toMatchObject({
      sourceProjectName: "E2E উৎস হিসাবখাতা",
    ...
    await expect(
      caller(outsider).finance.exportProjectBackup({
        projectId: sourceProjectId,
      })
    ).rejects.toBeTruthy();
    ...
    await expect(
      adminCaller.finance.restoreProjectBackup({
        projectName: "E2E পুনরুদ্ধার",
        confirmation: "NOT_CONFIRMED" as "RESTORE_NEW_PROJECT",
        backup,
      })
    ).rejects.toBeTruthy();
    expect(
      (await adminCaller.projects.list()).map(project => project.id)
    ).toEqual(projectsBeforeRejectedRestore.map(project => project.id));

    const transactionFailureBackup = structuredClone(backup);
    transactionFailureBackup.transactions[0].amount =
      "999999999999999999999999999999999.99";
    await expect(
      adminCaller.finance.restoreProjectBackup({
        projectName: "E2E রোলব্যাক",
        confirmation: "RESTORE_NEW_PROJECT",
        backup: transactionFailureBackup,
      })
    ).rejects.toBeTruthy();
    expect(
      (await adminCaller.projects.list()).map(project => project.id)
    ...
    const restoration = await adminCaller.finance.restoreProjectBackup({
      projectName: "E2E পুনরুদ্ধার",
      confirmation: "RESTORE_NEW_PROJECT",
      backup,
    });
    expect(restoration.projectId).not.toBe(sourceProjectId);
    const restoredBackup = await adminCaller.finance.exportProjectBackup({
    ...
    expect(restoredBackup.transactions).toHaveLength(
      backup.transactions.length
    );
    expect(
      (
        await adminCaller.finance.exportProjectBackup({
          projectId: sourceProjectId,
        })
      ).project.name
    ).toBe("E2E উৎস হিসাবখাতা");
    await expect(
      adminCaller.finance.restoreProjectBackup({
        projectName: "E2E পুনরুদ্ধার",
        confirmation: "RESTORE_NEW_PROJECT",
    ...
    ).rejects.toThrow("এই নামে একটি প্রজেক্ট ইতিমধ্যে আছে");
