...
beforeAll(async () => {
  ...
  await db.insert(users).values([
    {
      ...
      name: "RBAC Input Operator",
      email: "input@rbac.test",
      loginMethod: "e2e",
      role: "user",
      status: "active",
    },
    ...
    {
      openId: "rbac-legacy-admin",
      name: "RBAC Legacy Admin",
      email: "legacy@rbac.test",
      loginMethod: "e2e",
      role: "admin",
      status: "active",
    },
  ...
  if ([inputOperator, superAdmin, viewer, legacyAdmin].some(user => !user))
    throw new Error("RBAC E2E পরিচয় তৈরি করা যায়নি");

  await assignRole(
    inputOperator.id,
    ROLE_NAMES.INPUT_OPERATOR,
    inputOperator.id
  );
...
describe("real-stack RBAC E2E", () => {
  it("auth.me returns the strict INPUT_OPERATOR contract for an input operator", async () => {
    ...
    expect([...me!.permissions].sort()).toEqual(
      [...INPUT_OPERATOR_PERMISSIONS].sort()
    );
  ...
  it("auth.me exposes RBAC roles even when the legacy column is not admin", async () => {
    ...
    expect(me!.permissions).toEqual(
      expect.arrayContaining(["voucher.read", "voucher.post", "user.read"])
    );
  });
  ...
  it("denies read, update, and admin gates to the INPUT_OPERATOR", async () => {
    ...
    await expect(
      op.finance.overview({ projectId: project.id })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    // voucherList is denied by the voucher.read gate before any DB access.
    await expect(
      op.finance.voucherList({ projectId: project.id })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      op.finance.saveFirmProfile({ projectId: project.id, name: "X" })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });

    const adminPassword = process.env.ADMIN_ACCESS_PASSWORD;
    if (adminPassword) {
      await expect(op.admin.users()).rejects.toMatchObject({
        code: "FORBIDDEN",
      });
    }
  });
  ...
  it("grants financial admin gates through the RBAC SUPER_ADMIN role (legacy role not admin)", async () => {
    ...
    const project = await admin.projects.create({
      name: "RBAC E2E সুপার খাতা",
    });
    const overview = await admin.finance.overview({ projectId: project.id });
    expect(Array.isArray((overview as { accounts?: unknown[] }).accounts)).toBe(
      true
    );

    const adminPassword = process.env.ADMIN_ACCESS_PASSWORD;
    if (adminPassword) {
      const verified = await admin.admin.verifyAccess({
      ...
      await expect(
        ...
        expect.arrayContaining([
          expect.objectContaining({ email: "input@rbac.test", role: "user" }),
        ])
      );
    }
  });
