...
export const appRouter = router({
  ...
  finance: router({
    ...
    employeesList: protectedProcedure.input(z.object({ projectId })).query(({ ctx, input }) => financeDb.getEmployees(ctx.user.id, input.projectId)),
    createEmployee: protectedProcedure.input(z.object({
      ...
      notes: z.string().max(500).optional(),
    })).mutation(({ ctx, input }) => financeDb.createEmployee(ctx.user.id, input)),
    updateEmployee: protectedProcedure.input(z.object({
      projectId,
      id: z.number().int().positive(),
      name: z.string().trim().min(1).max(180).optional(),
      phone: z.string().trim().max(40).optional(),
      ...
      bankAccountDetails: z.string().max(500).optional(),
      notes: z.string().max(500).optional(),
    })).mutation(({ ctx, input }) => financeDb.updateEmployee(ctx.user.id, input.projectId, input.id, input)),
    deleteEmployee: protectedProcedure.input(z.object({ projectId, id: z.number().int().positive() })).mutation(({ ctx, input }) => financeDb.deleteEmployee(ctx.user.id, input.projectId, input.id)),
    salaryPaymentsList: protectedProcedure.input(z.object({
      projectId,
      monthKey: z.string().regex(/^\d{4}-\d{2}$/).optional(),
    })).query(({ ctx, input }) => financeDb.getSalaryPayments(ctx.user.id, input.projectId, input.monthKey)),
    disburseSalary: protectedProcedure.input(z.object({
      projectId,
      ...
      notes: z.string().max(500).optional(),
    })).mutation(({ ctx, input }) => financeDb.disburseSalary(ctx.user.id, input)),
    employeeAdvancesList: protectedProcedure.input(z.object({
      projectId,
      employeeId: z.number().int().positive().optional(),
    })).query(({ ctx, input }) => financeDb.getEmployeeAdvances(ctx.user.id, input.projectId, input.employeeId)),
    createEmployeeAdvance: protectedProcedure.input(z.object({
      projectId,
      employeeId: z.number().int().positive(),
