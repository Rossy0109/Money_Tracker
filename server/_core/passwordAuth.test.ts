...
describe("Constant-Time Credential Verification", () => {
  it("accepts the correct password against a real stored hash", async () => {
    const hash = await hashPassword("correctPassword");
    expect(await verifyPasswordConstantTime("correctPassword", hash)).toBe(true);
  });

  it("rejects a wrong password against a real stored hash", async () => {
    const hash = await hashPassword("correctPassword");
    expect(await verifyPasswordConstantTime("wrongPassword", hash)).toBe(false);
  });

  it("rejects login when no stored hash exists (e.g. unknown or OAuth-only account)", async () => {
    expect(await verifyPasswordConstantTime("whatever123", undefined)).toBe(false);
    expect(await verifyPasswordConstantTime("whatever123", null)).toBe(false);
  });
});
