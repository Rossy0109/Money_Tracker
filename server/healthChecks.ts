...
export interface HealthReport {
  ...
  integrity: Array<{ projectId: number; projectName: string; status: string; diffs: number; checkedAt: string }>;
}

function timed<T>(fn: () => Promise<T>): Promise<{ latencyMs: number; result: T }> {
  const start = Date.now();
  return fn().then(result => ({ latencyMs: Date.now() - start, result }));
}

export async function runHealthChecks(userId: number | null): Promise<HealthReport> {
  ...
  const dbWrite = await timed(async () => {
    await saveHealthSnapshot({ probeId: "database.write", status: "ok", latencyMs: 0 });
  });
  checks.push({
    id: "database.write",
    label: "ডেটাবেস লেখা",
  ...
  if (authMode === "google") {
    const configured =
      Boolean(ENV.googleOAuthClientId && ENV.googleOAuthClientSecret && ENV.googleOAuthRedirectUri);
    checks.push({
      id: "auth.google",
      label: "Google লগইন কনফিগারেশন",
      status: configured ? "ok" : "fail",
      timestamp: checkedAt,
      error: configured ? null : "GOOGLE_OAUTH_CLIENT_ID/SECRET/REDIRECT_URI সেট নেই",
      retryAction: configured ? undefined : "প্রজেক্ট এনভায়রনমেন্টে OAuth ক্রেডেনশিয়াল সেট করুন",
    });
  } else {
    checks.push({
      id: "auth.password",
  ...
  if (!supabase) {
    ...
    const probe = await timed(async () => {
      const response = await fetch(`${supabase.supabaseUrl}/storage/v1/bucket/${supabase.storageBucket ?? "amar-hisab-backups"}`, {
        headers: { apikey: supabase.supabaseAnonKey, Authorization: `Bearer ${supabase.supabaseAnonKey}` },
        signal: AbortSignal.timeout(10_000),
      });
      if (response.status === 404) throw new Error(`বাকেটের অস্তিত্ব নেই: ${supabase.storageBucket}`);
      if (response.status === 401 || response.status === 403) throw new Error("Supabase API key অবৈধ");
      if (!response.ok) throw new Error(`Supabase উত্তর দেয়নি (${response.status})`);
    });
    checks.push({
      id: "storage.supabase",
      label: "Supabase স্টোরেজ (ব্যাকআপ টার্গেট)",
  ...
  checks.push({
    ...
    retryAction: ENV.blobReadWriteToken ? undefined : "Vercel Blob কানেক্ট করুন",
  });
  ...
  if (!vercelUrl) {
    ...
    const probe = await timed(async () => {
      const response = await fetch(`${vercelUrl}/api/healthz`, { signal: AbortSignal.timeout(10_000) });
      if (!response.ok) throw new Error(`healthz উত্তর দেয়নি (${response.status})`);
    });
    checks.push({
      id: "vercel.api",
      label: "Vercel ডিপ্লয়মেন্ট / API",
  ...
  if (!userId) {
    ...
    const driveProbe = driveActive
      ? await timed(async () => {
          ...
          const root = await findDriveFileByName(token, connection.rootFolderName || "Money_Tracker", null, "application/vnd.google-apps.folder");
    ...
    checks.push({
      ...
      error: driveProbe ? null : "Google Drive সংযুক্ত নয়; System Health পেজে কানেক্ট করুন",
      retryAction: driveProbe ? undefined : "Drive সংযোগ করুন",
    });
  }

  const lastDatabaseBackup = userId ? await lastBackupForKind(userId, "database") : null;
  const lastNonDatabase = userId
    ? (await Promise.all(
        (["vouchers", "daily_statement", "monthly_statement", "documents"] as const).map(kind =>
          lastBackupForKind(userId, kind)
        )
      )).sort((a: { verifiedAt: Date | null } | null, b: { verifiedAt: Date | null } | null) => (b?.verifiedAt?.getTime() ?? 0) - (a?.verifiedAt?.getTime() ?? 0))[0]
    : null;

  const backupAgeHours = (verifiedAt: Date | null) =>
    verifiedAt ? (Date.now() - verifiedAt.getTime()) / 3_600_000 : Infinity;

  const lastBackupStatus: HealthSummary["lastBackup"] =
    !lastDatabaseBackup ? "none" : backupAgeHours(lastDatabaseBackup.verifiedAt) <= 48 ? "ok" : "stale";
  const lastSyncStatus: HealthSummary["lastSync"] =
    !lastNonDatabase ? "none" : backupAgeHours(lastNonDatabase.verifiedAt) <= 24 * 7 ? "ok" : "stale";

  checks.push({
    id: "backup.last",
    label: "সর্বশেষ ডেটাবেস ব্যাকআপ",
    status: lastBackupStatus === "none" ? "fail" : lastBackupStatus === "stale" ? "fail" : "ok",
    timestamp: checkedAt,
    error:
      !lastDatabaseBackup
  ...
  if (userId) {
    ...
    checks.push({
      ...
      timestamp: checkedAt,
      error: summary.failed > 0 ? `${summary.failed} ব্যর্থ` : summary.pending > 0 ? `${summary.pending} মুলতুবি` : null,
      retryAction:
  ...
  if (userId) {
    ...
    checks.push({
      ...
      status: results.every(r => r.status === "VERIFIED") ? "ok" : results.some(r => r.status === "MISMATCH") ? "fail" : "fail",
      timestamp: checkedAt,
      error:
