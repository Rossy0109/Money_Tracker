...
describe("Input-Only User Permission Model", () => {
  beforeEach(() => {
    ...
    rbacMock.hasAnyPermission.mockImplementation(
      async (userId: number, perms: string[]) => {
        if (userId === 42)
          return perms.some(p => INPUT_ONLY_PERMISSIONS.includes(p));
    ...
    rbacMock.hasAllPermissions.mockImplementation(
      async (userId: number, perms: string[]) => {
        if (userId === 42)
          return perms.every(p => INPUT_ONLY_PERMISSIONS.includes(p));
    ...
    rbacMock.isAdminRoleUser.mockImplementation(
  ...
  describe("Admin can do everything (regression)", () => {
    ...
    it("2. Admin can create (addTransaction)", async () => {
      financeDb.createTransaction.mockResolvedValue({
        id: 1,
        voucherNo: "V001",
      });
      const caller = appRouter.createCaller(adminContext);
  ...
  describe("Input-Only User can create via allowed procedures", () => {
    ...
    it("12. Can create Budget entry (saveBudget)", async () => {
      ...
      const caller = appRouter.createCaller(inputOnlyContext);
      const result = await caller.finance.saveBudget({
        projectId: 88,
        categoryId: 7,
        monthKey: "2026-08",
        amount: 5000,
    ...
    it("Can create Employee Advance (createEmployeeAdvance)", async () => {
      financeDb.createEmployeeAdvance.mockResolvedValue({
        id: 80,
        success: true,
        voucherNo: "EA080",
      });
  ...
  describe("Input-Only User MUST NOT list/read/search existing records", () => {
    ...
    it("Cannot read invoice by ID", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.finance.invoiceById({ projectId: 88, id: 1 })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
  ...
  describe("Input-Only User MUST NOT update existing records", () => {
    ...
    it("Cannot update employee", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.finance.updateEmployee({ projectId: 88, id: 1, name: "Updated" })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
    ...
    it("Cannot save firm profile", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.finance.saveFirmProfile({ projectId: 88, name: "Test" })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
    ...
    it("Cannot enable bill reminder", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.finance.enableBillReminder({
          projectId: 88,
          id: 1,
  ...
  describe("Input-Only User MUST NOT access reports/export/import", () => {
    ...
    it("25. Cannot access other users' records", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.finance.overview({ projectId: 999 })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
  ...
  describe("Input-Only User MUST NOT manage users/roles/permissions", () => {
    ...
    it("36. Cannot manage roles (admin.updateUserStatus)", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.admin.updateUserStatus({ targetUserId: 99, status: "active" })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
    ...
    it("39. Cannot access admin audit logs", async () => {
      const caller = appRouter.createCaller(inputOnlyContext);
      await expect(
        caller.admin.auditLogs({ page: 1, pageSize: 25 })
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
    });
  ...
  describe("Security — Client-supplied role/userId cannot elevate", () => {
    ...
    it("47. Request body createdBy cannot spoof another user", async () => {
      ...
      expect(financeDb.createTransaction).toHaveBeenCalledWith(
        42,
        expect.any(Object)
      );
    });
