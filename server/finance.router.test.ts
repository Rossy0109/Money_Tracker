...
describe("finance router", () => {
  ...
  it("recalculates a category's current-month alert after its budget is edited", () => {
    ...
    expect(
      calculateBudgetEarlyWarnings([{ ...existingBudget, spent: 4800 }])
    ).toMatchObject([{ categoryId: 7, threshold: 90 }]);
    expect(
      calculateBudgetEarlyWarnings([{ ...editedBudget, spent: 4800 }])
  ...
  it("saves an edited project budget and returns the recalculated alert state on overview refetch", async () => {
    ...
    financeDb.upsertBudget.mockImplementation(
      async (_userId: number, input: { amount: number }) => {
        savedBudgetAmount = input.amount;
        return { id: 11, ...input };
      }
    );
    financeDb.getOverview.mockImplementation(
      async (_userId: number, projectId: number) => ({
        projectId,
  ...
  it("exports, previews, and restores a validated backup only through a new project", async () => {
    ...
    const preview = {
      ...
      restorePolicy: "new-project-only",
    };
    ...
    financeDb.restoreProjectBackup.mockResolvedValue({
      projectId: 99,
      projectName: "পুনরুদ্ধারকৃত হিসাব",
    });
    const caller = appRouter.createCaller(authenticatedContext);
    ...
    await expect(
      caller.finance.previewProjectBackup({ backup })
    ...
    ).resolves.toEqual({ projectId: 99, projectName: "পুনরুদ্ধারকৃত হিসাব" });
    await expect(
      caller.finance.restoreProjectBackup({
        projectName: "পুনরুদ্ধারকৃত হিসাব",
    ...
    expect(financeDb.restoreProjectBackup).toHaveBeenCalledWith(42, {
      projectName: "পুনরুদ্ধারকৃত হিসাব",
      backup,
    });
  });
  ...
  it("keeps household profiles, invitation inboxes, roles, and shared budgets scoped to the signed-in member", async () => {
    ...
    financeDb.listHouseholdInvitations.mockResolvedValue([
      {
        membershipId: 41,
        householdId: 19,
        householdName: "আপন পরিবার",
        role: "viewer",
      },
    ]);
    financeDb.getHouseholdOverview.mockResolvedValue({
      household,
      members: [],
      budgets: [],
      expenses: [],
    ...
    financeDb.updateHouseholdMember.mockResolvedValue({
      membershipId: 42,
      role: "viewer",
    });
    ...
    await expect(caller.finance.householdInvitations()).resolves.toHaveLength(
      1
    );
    await expect(
      caller.finance.createHousehold({ name: "আমাদের পরিবার" })
    ).resolves.toEqual(household);
    await expect(
      caller.finance.householdOverview({ householdId: 17 })
    ).resolves.toMatchObject({ household });
    await caller.finance.inviteHouseholdMember({
      householdId: 17,
    ...
    await caller.finance.updateHouseholdMember({
      householdId: 17,
      membershipId: 42,
      role: "viewer",
    });
    await caller.finance.saveSharedHouseholdBudget({
      householdId: 17,
      label: "খাবার",
      monthKey: "2026-08",
      amount: 10000,
    });
    await caller.finance.addSharedHouseholdExpense({
      householdId: 17,
      budgetId: 31,
      amount: 1200,
    ...
    expect(financeDb.inviteHouseholdMember).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ householdId: 17, role: "editor" })
    );
    expect(financeDb.acceptHouseholdInvitation).toHaveBeenCalledWith(42, 41);
    expect(financeDb.updateHouseholdMember).toHaveBeenCalledWith(
      42,
    ...
    expect(financeDb.saveSharedBudget).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ householdId: 17, monthKey: "2026-08" })
    );
    expect(financeDb.addSharedExpense).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ householdId: 17, budgetId: 31 })
    );
    await expect(
      caller.finance.inviteHouseholdMember({
        householdId: 17,
        email: "member@example.com",
        role: "owner",
      } as any)
  ...
  it("keeps automation, due dates, and recurring template actions scoped to the selected project", async () => {
    ...
    ).resolves.toEqual({ recurring: [], bills: [], ageing: [] });
    await caller.finance.addDue({
      projectId: 88,
      type: "debt",
      counterparty: "সরবরাহকারী",
      amount: 1200,
      openedAt: new Date("2026-08-20T12:00:00Z"),
      dueAt: new Date("2026-09-01T12:00:00Z"),
    });
    await caller.finance.addRecurringTemplate({
      ...expenseInput,
      frequency: "monthly",
      scheduleDay: 15,
      nextRunAt: new Date("2026-09-15T12:00:00Z"),
    });
    expect(financeDb.getAutomationOverview).toHaveBeenCalledWith(42, 88);
    expect(financeDb.createDue).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ projectId: 88, dueAt: expect.any(Date) })
    );
    expect(financeDb.createRecurringTemplate).toHaveBeenCalledWith(
      42,
      expect.objectContaining({
        projectId: 88,
        frequency: "monthly",
  ...
  it("returns planning and analytics only for the authenticated user's selected project", async () => {
    const plan = {
      ...
      plans: [{ categoryId: 7, suggestedAmount: 5000 }],
    };
    const analytics = {
      data: [
        {
    ...
    await expect(
      caller.finance.budgetPlan({ projectId: 88, monthKey: "2026-09" })
  ...
  it("passes validated transaction search filters through the authenticated user and project boundary", async () => {
    ...
    await expect(
      appRouter
        .createCaller(authenticatedContext)
        .finance.searchTransactions(input)
    ).resolves.toEqual(results);
    expect(financeDb.searchTransactions).toHaveBeenCalledWith(42, input);
    await expect(
      appRouter
        .createCaller(authenticatedContext)
        .finance.searchTransactions({ ...input, from: to, to: from })
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  });
  ...
  it("passes validated paginated transaction filters and returns SQL aggregations", async () => {
    ...
    await expect(
      appRouter
        .createCaller(authenticatedContext)
        .finance.paginatedTransactions(input)
  ...
  it("scopes a monthly financial report to the authenticated user, project, and validated month", async () => {
    const report = {
      ...
      previousExpenseCategoryTotals: [{ name: "বেতন", total: 3000 }],
      ...
      financialPosition: {
        ...
        assets: 6000,
        debts: 0,
        netFinancialPosition: 6000,
      },
      accountDetails: [{ name: "নগদ", type: "cash", currentBalance: 6000 }],
      dueDetails: [],
      transactionDetails: [
        {
    ...
    await expect(
      appRouter
        .createCaller(authenticatedContext)
        .finance.monthlyReport({ projectId: 88, monthKey: "2026-08" })
    ...
    expect(report.previousExpenseCategoryTotals).toEqual([
      { name: "বেতন", total: 3000 },
    ]);
    await expect(
      appRouter
        .createCaller(authenticatedContext)
        .finance.monthlyReport({ projectId: 88, monthKey: "2026-13" })
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  });

  it("exports only the authenticated user's finance data", async () => {
    financeDb.exportUserData.mockResolvedValue({
      projects: [],
      transactions: [],
    });
  ...
  it("scopes a new transaction and an edit to the authenticated user and project", async () => {
    ...
    await caller.finance.updateTransaction({
      id: 501,
      ...expenseInput,
      amount: 1650,
    });
    expect(financeDb.createTransaction).toHaveBeenCalledWith(42, expenseInput);
    expect(financeDb.updateTransaction).toHaveBeenCalledWith(42, 501, {
      ...expenseInput,
      amount: 1650,
    });
  });
  ...
  it("uses the description-only transaction contract while voucher numbering remains server-side", async () => {
    await appRouter
      .createCaller(authenticatedContext)
      .finance.addTransaction(expenseInput);
  ...
  it("gets and saves voucher settings only within the authenticated user's project", async () => {
    ...
    await expect(
      caller.finance.saveVoucherSettings(input)
    ).resolves.toMatchObject({
      prefix: "EXP",
      startNumber: 10,
      endNumber: 999,
  ...
  it("creates a new isolated project for the authenticated user", async () => {
    ...
    await appRouter
      .createCaller(authenticatedContext)
      .projects.create({ name: "নতুন প্রকল্প" });
  ...
  it("scopes account edits and deletions to the authenticated user and project", async () => {
    const caller = appRouter.createCaller(authenticatedContext);
    await caller.finance.updateAccount({
      id: 16,
      projectId: 88,
      name: "Cash",
      type: "cash",
    ...
    expect(financeDb.updateAccount).toHaveBeenCalledWith(42, 16, {
      projectId: 88,
      name: "Cash",
      type: "cash",
      openingBalance: 4000,
  ...
  it("rejects unauthenticated finance requests", async () => {
    await expect(
      appRouter
        .createCaller({ ...authenticatedContext, user: null })
        .finance.overview({ projectId: 88 })
  ...
  it("does not permit a standard user to inspect administrator audit logs", async () => {
    await expect(
      appRouter.createCaller(authenticatedContext).admin.auditLogs({})
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      appRouter.createCaller(authenticatedContext).admin.auditLogExport({})
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  ...
  it("permits a verified administrator to inspect the audit log", async () => {
    financeDb.listAuditLogsPage.mockResolvedValue({
      logs: [{ id: 1, summary: "Transaction created" }],
      page: 1,
      pageSize: 25,
      total: 1,
      totalPages: 1,
    ...
    await expect(
      caller.admin.verifyAccess({ password: ENV.adminAccessPassword })
    ).resolves.toMatchObject({ verified: true });
    await expect(caller.admin.auditLogs({})).resolves.toMatchObject({
      logs: [{ id: 1, summary: "Transaction created" }],
      page: 1,
      pageSize: 25,
    });
    expect(financeDb.listAuditLogsPage).toHaveBeenCalledWith({
      from: undefined,
      to: undefined,
  ...
  it("permits a verified administrator to retrieve only the selected audit-log export set", async () => {
    financeDb.listAuditLogsForExport.mockResolvedValue([
      { id: 2, summary: "Transaction deleted" },
    ]);
    const caller = appRouter.createCaller(administratorContext);

    await expect(
      caller.admin.auditLogExport({
        actorUserId: 17,
        actorRole: "user",
        search: "deleted",
      })
    ).resolves.toEqual([{ id: 2, summary: "Transaction deleted" }]);
    expect(financeDb.listAuditLogsForExport).toHaveBeenCalledWith({
      from: undefined,
      to: undefined,
      actorUserId: 17,
      actorRole: "user",
  ...
  it("returns filtered audit activity analytics only after administrator verification", async () => {
    ...
    await expect(
      caller.admin.auditActivity({ actorRole: "admin" })
    ).resolves.toEqual([{ action: "update", count: 8 }]);
    expect(financeDb.getAuditLogActivity).toHaveBeenCalledWith({
      from: undefined,
      to: undefined,
      actorUserId: undefined,
      actorRole: "admin",
    ...
    await expect(
      appRouter.createCaller(authenticatedContext).admin.auditActivity({})
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  });
  ...
  it("permits a verified administrator to inspect all registered project workspaces", async () => {
    financeDb.listProjectsForAdmin.mockResolvedValue([
      { id: 88, name: "দৈনিক লেনদেনের খাতা" },
    ]);
    const caller = appRouter.createCaller(administratorContext);

    await expect(caller.admin.projects()).resolves.toEqual([
      { id: 88, name: "দৈনিক লেনদেনের খাতা" },
    ]);
