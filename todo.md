...
- [ ] Implement the server-side Google authorization-code flow with PKCE, state, nonce, discovery/JWKS ID-token verification, and `google:<sub>` identity mapping.
- [ ] Add mode-switch and Google OAuth regression tests, including invalid state, nonce, issuer, audience, expired-token, and verified-email/admin-bootstrap denial paths.
