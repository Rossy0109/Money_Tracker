...
if (process.platform === "android") {
  throw new Error(
    "Browser E2E requires Linux, macOS, or Windows; run it in CI or a supported host"
  );
}
...
if (
  ...
  source.pathname.replace(/^\//, "").startsWith("money_tracker_e2e_") ||
  !["127.0.0.1", "localhost"].includes(source.hostname)
) {
  throw new Error(
    "নিরাপত্তার জন্য ব্রাউজার E2E কেবল স্থানীয় অ-পরীক্ষামূলক MariaDB সংযোগ ব্যবহার করবে"
  );
}

function quoteIdentifier(identifier) {
  if (!/^money_tracker_e2e_browser_[a-z0-9_]{8,60}$/.test(identifier))
    throw new Error("নিরাপদ ব্রাউজার E2E ডেটাবেসের নাম তৈরি করা যায়নি");
  return `\`${identifier}\``;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", code =>
      code === 0
        ...
        : reject(
            new Error(
              `${command} ${args.join(" ")} ব্যর্থ হয়েছে (exit ${code ?? "unknown"})`
            )
          )
    );
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, "127.0.0.1", () => {
      ...
      if (!address || typeof address === "string") {
        probe.close();
        reject(
          new Error("ব্রাউজার E2E-এর জন্য খালি লুপব্যাক পোর্ট পাওয়া যায়নি")
        );
        return;
      }
      probe.close(error =>
...
try {
  ...
  if (testDatabaseCreated) {
    ...
    try {
      ...
      console.log(
        `পরীক্ষামূলক ব্রাউজার E2E ডেটাবেস নিরাপদে অপসারণ করা হয়েছে: ${testDatabase}`
      );
    } finally {
      adminConnection.destroy();
    }
