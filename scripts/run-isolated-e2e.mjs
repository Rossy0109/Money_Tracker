...
if (
  ...
  sourceDatabase.startsWith("money_tracker_e2e_") ||
  !["127.0.0.1", "localhost"].includes(source.hostname)
) {
  throw new Error(
    "নিরাপত্তার জন্য E2E রানার কেবল স্থানীয় অ-পরীক্ষামূলক MariaDB সংযোগ ব্যবহার করবে"
  );
}
...
try {
  ...
  if (testDatabaseCreated) {
    ...
    try {
      ...
      console.log(
        `পরীক্ষামূলক ডেটাবেস নিরাপদে অপসারণ করা হয়েছে: ${testDatabase}`
      );
    } finally {
      adminConnection.destroy();
    }
